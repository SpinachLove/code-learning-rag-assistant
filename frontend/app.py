...
with col_code:
    ...
    code_content = st.text_area(
        "请粘贴你要分析的代码",
        height=300,
        placeholder="在这里输入你的代码...", #预设文字
        key="code_input"
    )
    # 功能按钮
    analyze_btn = st.button(      #调用组件
        "开始分析代码",
        type="primary",
...
# 用户提问输入框
user_question = st.chat_input("请输入你的编程问题，例如：Python如何处理异常？")    #不输入内容无法发送（chat_input组件自带内容）
    ...
    with st.chat_message("assistant"):
        with st.spinner("正在检索知识库并生成答案..."):
            try:
                # 调用后端问答接口
                response = requests.post(
                    url=f"{BACKEND_BASE_URL}/api/rag-qa", #BACKEND_BASE_URL = "http://127.0.0.1:8000" 后端接口地址
                    json={
                ...
                if response.status_code == 200:
                    ...
                    st.markdown(answer)
                    # 展示引用内容
                    with st.expander("查看引用的知识库内容", expanded=False):  #设置折叠面板
