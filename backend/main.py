...
from rag.vector_db import similarity_search     #从别的文件夹导入函数
...
@app.post("/api/rag-qa", tags=["核心功能接口"])          #调用到这个后，先判断是否为空
async def rag_qa(request: RagQARequest):
    if not request.question.strip():
        raise HTTPException(status_code=400, detail="问题不能为空")
    try:
        # 调用问答链
        result = rag_qa_chain(request.question, request.code_context)     #问题不为空调用问答链
        return {
            "code": 200,
            "message": "问答生成成功",
            "data": result
